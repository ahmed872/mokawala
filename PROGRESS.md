...
16. **إصلاح كراش الدخول "Invalid salt" (2026-07-06):** حساب متسجّل بهاش كلمة مرور فارغ كان يرمي استثناء من BCrypt.Verify ويمنع الدخول. `AuthenticateAsync` بقى يعالج الهاش الفارغ/التالف: يقبل الدخول بكلمة المرور الافتراضية = اسم المستخدم ويطلب تغييرها؛ + backfill في `DataBootstrapService` يصلح كل الحسابات الفارغة أول تشغيل.
17. **وصول "عهدتي" (2026-07-06):** التابات في MainWindow **مخفية** (`HiddenTabControlStyle`) والتنقل بكروت لوحة التحكم — فتبويب "عهدتي" الجديد ماكانش ليه كارت. اتضاف كارت `DashboardMyCustodyShortcutButton` (ظاهر للكل) + `QuickOpenMyCustodyButton_Click` → `SelectTabByTag("MyCustody")`. **درس:** أي تبويب جديد في MainWindow لازم يتوصله كارت/زر تنقل، مش مجرد TabItem.
18. **اعتماد مرتجع العهدة (2026-07-06):** حركة الخزينة لها `Status` ("معتمد" افتراضي / "معلق"). مرتجع تصفية العهدة بيتسجل "معلق" ولا يدخل الرصيد ولا الإجماليات حتى يعتمده مسؤول الخزينة أو المدير من زر "اعتماد" في تبويب الخزينة (بيظهر فقط للحركات المعلقة). `TreasuryService.ApproveAsync` + عمود Status في TreasuryTransactions (EnsureColumn) + استثناء المعلق من `GetCurrentBalanceAsync` وداشبورد `GetDashboardMetricsAsync` و`UpdateTreasurySummary`. تعديل حركة معلقة لا يعتمدها ضمنيًا.
